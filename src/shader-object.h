...
public:
    ...
    /// Minimum total size (in bytes) required for the uniform/ordinary-data
    /// buffer backing this layout. Backends that bind resources via uniform
    /// marshaling (CUDA) use this to widen storage for synthesized resources
    /// whose uniform offset lies outside the Slang element-type layout.
    /// Callers should allocate `max(elementTypeSize, getMinUniformBufferSize())`.
