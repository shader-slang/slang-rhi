...
public:
    ...
    WebGPUCommandEncoder m_commandEncoder = nullptr;
    WebGPURenderPassEncoder m_renderPassEncoder = nullptr;
    WebGPUComputePassEncoder m_computePassEncoder = nullptr;
...
Result CommandRecorder::record(CommandBufferImpl* commandBuffer)
{
    ...
    m_commandEncoder = m_ctx.api.webgpuDeviceCreateCommandEncoder(m_ctx.device, nullptr);
    SLANG_RHI_DEFERRED({ m_ctx.api.webgpuCommandEncoderRelease(m_commandEncoder); });
    if (!m_commandEncoder)
    {
        return SLANG_FAIL;
    }
...
#undef SLANG_RHI_COMMAND_EXECUTE_X
    ...
    commandBuffer->m_commandBuffer = m_ctx.api.webgpuCommandEncoderFinish(m_commandEncoder, nullptr);
...
void CommandRecorder::cmdCopyBuffer(const commands::CopyBuffer& cmd)
{
    ...
    m_ctx.api.webgpuCommandEncoderCopyBufferToBuffer(
        m_commandEncoder,
        src->m_buffer,
        cmd.srcOffset,
        dst->m_buffer,
...
void CommandRecorder::cmdCopyTexture(const commands::CopyTexture& cmd)
{
    ...
    for (uint32_t layer = 0; layer < dstSubresource.layerCount; layer++)
    {
        for (uint32_t mip = 0; mip < dstSubresource.mipCount; mip++)
        {
            ...
            WebGPUImageCopyTexture source = {};
            source.texture = src->m_texture;
            source.origin = {cmd.srcOffset.x, cmd.srcOffset.y, srcZ};
            source.mipLevel = srcMip;
            source.aspect = WebGPUTextureAspect_All;

            WebGPUImageCopyTexture destination = {};
            destination.texture = dst->m_texture;
            destination.origin = {cmd.dstOffset.x, cmd.dstOffset.y, dstZ};
            destination.mipLevel = dstMip;
            destination.aspect = WebGPUTextureAspect_All;

            WebGPUExtent3D copySize = {adjustedExtent.width, adjustedExtent.height, adjustedExtent.depth};
            ...
            m_ctx.api.webgpuCommandEncoderCopyTextureToTexture(m_commandEncoder, &source, &destination, &copySize);
        }
    }
}
...
void CommandRecorder::cmdCopyTextureToBuffer(const commands::CopyTextureToBuffer& cmd)
{
    ...
    WebGPUImageCopyBuffer destination = {};
    ...
    m_ctx.api.webgpuCommandEncoderCopyTextureToBuffer(m_commandEncoder, &source, &destination, &copySize);
}

void CommandRecorder::cmdClearBuffer(const commands::ClearBuffer& cmd)
{
    BufferImpl* buffer = checked_cast<BufferImpl*>(cmd.buffer);
    m_ctx.api.webgpuCommandEncoderClearBuffer(m_commandEncoder, buffer->m_buffer, cmd.range.offset, cmd.range.size);
}
...
void CommandRecorder::cmdUploadTextureData(const commands::UploadTextureData& cmd)
{
    ...
    for (uint32_t layerOffset = 0; layerOffset < subresourceRange.layerCount; layerOffset++)
    {
        ...
        for (uint32_t mipOffset = 0; mipOffset < subresourceRange.mipCount; mipOffset++)
        {
            ...
            WebGPUImageCopyBuffer srcRegion;
            ...
            WebGPUImageCopyTexture dstRegion;
            dstRegion.aspect = WebGPUTextureAspect_All;
            ...
            m_ctx.api.webgpuCommandEncoderCopyBufferToTexture(m_commandEncoder, &srcRegion, &dstRegion, &copySize);
...
void CommandRecorder::cmdBeginRenderPass(const commands::BeginRenderPass& cmd)
{
    ...
    short_vector<WebGPURenderPassColorAttachment, 8> colorAttachments(desc.colorAttachmentCount, {});
    for (uint32_t i = 0; i < desc.colorAttachmentCount; ++i)
    {
        const RenderPassColorAttachment& attachmentIn = desc.colorAttachments[i];
        WebGPURenderPassColorAttachment& attachment = colorAttachments[i];
    ...
    WebGPURenderPassDepthStencilAttachment depthStencilAttachment = {};
    if (desc.depthStencilAttachment)
    {
        const RenderPassDepthStencilAttachment& attachmentIn = *desc.depthStencilAttachment;
        WebGPURenderPassDepthStencilAttachment& attachment = depthStencilAttachment;
        attachment.view = checked_cast<TextureViewImpl*>(attachmentIn.view)->m_textureView;
    ...
    WebGPURenderPassDescriptor passDesc = {};
    ...
    endPassEncoder();
    m_renderPassEncoder = m_ctx.api.webgpuCommandEncoderBeginRenderPass(m_commandEncoder, &passDesc);
...
void CommandRecorder::cmdSetRenderState(const commands::SetRenderState& cmd)
{
    ...
    WebGPURenderPassEncoder encoder = m_renderPassEncoder;

    if (updatePipeline)
    {
        m_renderPipeline = checked_cast<RenderPipelineImpl*>(cmd.pipeline);
        m_ctx.api.webgpuRenderPassEncoderSetPipeline(encoder, m_renderPipeline->m_renderPipeline);
    }
    ...
    if (updateBindings)
    {
        ...
        {
            m_ctx.api.webgpuRenderPassEncoderSetBindGroup(
                m_renderPassEncoder,
                groupIndex,
    ...
    if (updateStencilRef)
    {
        m_ctx.api.webgpuRenderPassEncoderSetStencilReference(m_renderPassEncoder, state.stencilRef);
    }
    ...
    if (updateVertexBuffers)
    {
        for (uint32_t i = 0; i < state.vertexBufferCount; ++i)
        {
            ...
            m_ctx.api.webgpuRenderPassEncoderSetVertexBuffer(
                m_renderPassEncoder,
                i,
                buffer->m_buffer,
                offset,
    ...
    if (updateIndexBuffer)
    {
        if (state.indexBuffer.buffer)
        {
            ...
            m_ctx.api.webgpuRenderPassEncoderSetIndexBuffer(
                m_renderPassEncoder,
                buffer->m_buffer,
                state.indexFormat == IndexFormat::Uint32 ? WebGPUIndexFormat_Uint32 : WebGPUIndexFormat_Uint16,
                offset,
                buffer->m_desc.size - offset
            );
        }
    ...
    if (updateViewports && state.viewportCount > 0)
    {
        ...
        m_ctx.api.webgpuRenderPassEncoderSetViewport(
            m_renderPassEncoder,
            viewport.originX,
            viewport.originY,
            viewport.extentX,
    ...
    if (updateScissorRects && state.scissorRectCount > 0)
    {
        ...
        m_ctx.api.webgpuRenderPassEncoderSetScissorRect(
            m_renderPassEncoder,
            scissorRect.minX,
            scissorRect.minY,
...
void CommandRecorder::cmdDraw(const commands::Draw& cmd)
{
    ...
    m_ctx.api.webgpuRenderPassEncoderDraw(
        m_renderPassEncoder,
...
void CommandRecorder::cmdDrawIndexed(const commands::DrawIndexed& cmd)
{
    ...
    m_ctx.api.webgpuRenderPassEncoderDrawIndexed(
        m_renderPassEncoder,
        cmd.args.vertexCount,
        cmd.args.instanceCount,
...
void CommandRecorder::cmdDrawIndirect(const commands::DrawIndirect& cmd)
{
    ...
    m_ctx.api.webgpuRenderPassEncoderMultiDrawIndirect(
        m_renderPassEncoder,
...
void CommandRecorder::cmdDrawIndexedIndirect(const commands::DrawIndexedIndirect& cmd)
{
    ...
    m_ctx.api.webgpuRenderPassEncoderMultiDrawIndexedIndirect(
        m_renderPassEncoder,
        checked_cast<BufferImpl*>(cmd.argBuffer.buffer)->m_buffer,
        cmd.argBuffer.offset,
        cmd.maxDrawCount,
...
{
    endPassEncoder();
    m_computePassEncoder = m_ctx.api.webgpuCommandEncoderBeginComputePass(m_commandEncoder, nullptr);
...
void CommandRecorder::cmdSetComputeState(const commands::SetComputeState& cmd)
{
    ...
    WebGPUComputePassEncoder encoder = m_computePassEncoder;

    if (updatePipeline)
    {
        m_computePipeline = checked_cast<ComputePipelineImpl*>(cmd.pipeline);
        m_ctx.api.webgpuComputePassEncoderSetPipeline(encoder, m_computePipeline->m_computePipeline);
    }
    ...
    if (updateBindings)
    {
        ...
        for (uint32_t groupIndex = 0; groupIndex < m_bindingData->bindGroupCount; groupIndex++)
        {
            m_ctx.api.webgpuComputePassEncoderSetBindGroup(
...
void CommandRecorder::cmdDispatchCompute(const commands::DispatchCompute& cmd)
{
    ...
    m_ctx.api.webgpuComputePassEncoderDispatchWorkgroups(m_computePassEncoder, cmd.x, cmd.y, cmd.z);
}
...
void CommandRecorder::cmdDispatchComputeIndirect(const commands::DispatchComputeIndirect& cmd)
{
    ...
    m_ctx.api.webgpuComputePassEncoderDispatchWorkgroupsIndirect(
        m_computePassEncoder,
        checked_cast<BufferImpl*>(cmd.argBuffer.buffer)->m_buffer,
        cmd.argBuffer.offset
    );
...
void CommandRecorder::cmdPushDebugGroup(const commands::PushDebugGroup& cmd)
{
    ...
    {
        m_ctx.api.webgpuRenderPassEncoderPushDebugGroup(m_renderPassEncoder, cmd.name);
    }
    else if (m_computePassActive)
    {
        m_ctx.api.webgpuComputePassEncoderPushDebugGroup(m_computePassEncoder, cmd.name);
    }
    else
    {
        m_ctx.api.webgpuCommandEncoderPushDebugGroup(m_commandEncoder, cmd.name);
    }
}
...
void CommandRecorder::cmdPopDebugGroup(const commands::PopDebugGroup& cmd)
{
    if (m_renderPassActive)
    {
        m_ctx.api.webgpuRenderPassEncoderPopDebugGroup(m_renderPassEncoder);
    }
    else if (m_computePassActive)
    {
        m_ctx.api.webgpuComputePassEncoderPopDebugGroup(m_computePassEncoder);
    }
    else
    {
        m_ctx.api.webgpuCommandEncoderPopDebugGroup(m_commandEncoder);
    }
}
...
void CommandRecorder::cmdInsertDebugMarker(const commands::InsertDebugMarker& cmd)
{
    if (m_renderPassActive)
    {
        m_ctx.api.webgpuRenderPassEncoderInsertDebugMarker(m_renderPassEncoder, cmd.name);
    }
    else if (m_computePassActive)
    {
        m_ctx.api.webgpuComputePassEncoderInsertDebugMarker(m_computePassEncoder, cmd.name);
    }
    else
    {
        m_ctx.api.webgpuCommandEncoderInsertDebugMarker(m_commandEncoder, cmd.name);
    }
}
...
void CommandRecorder::endPassEncoder()
{
    if (m_renderPassEncoder)
    {
        m_device->m_ctx.api.webgpuRenderPassEncoderEnd(m_renderPassEncoder);
        m_device->m_ctx.api.webgpuRenderPassEncoderRelease(m_renderPassEncoder);
    ...
    {
        m_device->m_ctx.api.webgpuComputePassEncoderEnd(m_computePassEncoder);
        m_device->m_ctx.api.webgpuComputePassEncoderRelease(m_computePassEncoder);
    ...
    : CommandQueue(device_, type)
{
    ...
    m_queue = device->m_ctx.api.webgpuDeviceGetQueue(device->m_ctx.device);
}
...
CommandQueueImpl::~CommandQueueImpl()
{
    ...
    if (m_queue)
    {
        device->m_ctx.api.webgpuQueueRelease(m_queue);
    }
}
...
Result CommandQueueImpl::submit(const SubmitDesc& desc)
{
    ...
    short_vector<WebGPUCommandBuffer, 16> commandBuffers;
    for (uint32_t i = 0; i < desc.commandBufferCount; i++)
    {
        commandBuffers.push_back(checked_cast<CommandBufferImpl*>(desc.commandBuffers[i])->m_commandBuffer);
    }
    device->m_ctx.api.webgpuQueueSubmit(m_queue, commandBuffers.size(), commandBuffers.data());
...
Result CommandQueueImpl::waitOnHost()
{
    ...
    // Wait for the command buffer to finish executing
    {
        ...
        WebGPUFuture future = device->m_ctx.api.webgpuQueueOnSubmittedWorkDone2(m_queue, callbackInfo);
        constexpr size_t futureCount = 1;
        WebGPUFutureWaitInfo futures[futureCount] = {{future}};
        uint64_t timeoutNS = UINT64_MAX;
        WebGPUWaitStatus waitStatus =
            device->m_ctx.api.webgpuInstanceWaitAny(device->m_ctx.instance, futureCount, futures, timeoutNS);
...
{
    outHandle->type = NativeHandleType::WebGPUQueue;
...
CommandBufferImpl::~CommandBufferImpl()
{
    ...
    if (m_commandBuffer)
    {
        device->m_ctx.api.webgpuCommandBufferRelease(m_commandBuffer);
    }
}
...
{
    outHandle->type = NativeHandleType::WebGPUCommandBuffer;
