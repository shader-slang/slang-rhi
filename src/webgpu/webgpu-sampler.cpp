...
SamplerImpl::~SamplerImpl()
{
    if (m_sampler)
    {
        getDevice<DeviceImpl>()->m_ctx.api.webgpuSamplerRelease(m_sampler);
    }
}

Result SamplerImpl::getNativeHandle(NativeHandle* outHandle)
{
    outHandle->type = NativeHandleType::WebGPUSampler;
...
Result DeviceImpl::createSampler(const SamplerDesc& desc, ISampler** outSampler)
{
    ...
    WebGPUSamplerDescriptor samplerDesc = {};
    ...
    sampler->m_sampler = m_ctx.api.webgpuDeviceCreateSampler(m_ctx.device, &samplerDesc);
