...
#include "webgpu-sampler.h"
#include "webgpu-command.h"

namespace rhi::webgpu {

inline void writeDescriptor(BindingDataBuilder& builder, uint32_t bindingSet, const WebGPUBindGroupEntry& write)
{
...
)
{
    ...
    {
        m_entries.push_back(std::vector<WebGPUBindGroupEntry>());
...
Result BindingDataBuilder::createBindGroups()
{
    ...
    m_bindingData->bindGroups = m_allocator->allocate<WebGPUBindGroup>(m_bindingData->bindGroupCount);

    for (size_t i = 0; i < m_entries.size(); ++i)
    {
        WebGPUBindGroupDescriptor desc = {};
        desc.layout = m_bindGroupLayouts[i];
        desc.entries = m_entries[i].data();
        desc.entryCount = (uint32_t)m_entries[i].size();
        WebGPUBindGroup bindGroup = m_device->m_ctx.api.webgpuDeviceCreateBindGroup(m_device->m_ctx.device, &desc);
        if (!bindGroup)
        {
            return SLANG_FAIL;
        }
...
void BindingDataImpl::release(DeviceImpl* device)
{
    ...
    {
        device->m_ctx.api.webgpuBindGroupRelease(bindGroups[i]);
    }
}
