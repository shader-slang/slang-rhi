...
{
    API api;
    WebGPUInstance instance = nullptr;
    WebGPUAdapter adapter = nullptr;
    WebGPUDevice device = nullptr;
    WebGPULimits limits = {};
    std::unordered_set<WebGPUFeatureName> features;
...
public:
    ...
    void handleError(WebGPUErrorType type, const char* message);
    WebGPUErrorType getAndClearLastError();
...
private:
    WebGPUErrorType m_lastError = WebGPUErrorType_NoError;
