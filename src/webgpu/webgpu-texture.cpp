...
TextureImpl::~TextureImpl()
{
    ...
    if (m_texture)
    {
        getDevice<DeviceImpl>()->m_ctx.api.webgpuTextureRelease(m_texture);
    }
}

Result TextureImpl::getNativeHandle(NativeHandle* outHandle)
{
    outHandle->type = NativeHandleType::WebGPUTexture;
...
Result DeviceImpl::createTexture(const TextureDesc& desc_, const SubresourceData* initData, ITexture** outTexture)
{
    ...
    WebGPUTextureDescriptor textureDesc = {};
    ...
    if (initData)
    {
        textureDesc.usage |= WebGPUTextureUsage_CopyDst;
    }
    ...
    case TextureType::Texture1D:
        ...
        textureDesc.dimension = WebGPUTextureDimension_1D;
        break;
    ...
    case TextureType::TextureCubeArray:
        textureDesc.dimension = WebGPUTextureDimension_2D;
        break;
    case TextureType::Texture3D:
        textureDesc.dimension = WebGPUTextureDimension_3D;
        textureDesc.size.depthOrArrayLayers = desc.size.depth;
        break;
    }

    texture->m_texture = m_ctx.api.webgpuDeviceCreateTexture(m_ctx.device, &textureDesc);
...
TextureViewImpl::~TextureViewImpl()
{
    if (m_textureView)
    {
        getDevice<DeviceImpl>()->m_ctx.api.webgpuTextureViewRelease(m_textureView);
    }
}

Result TextureViewImpl::getNativeHandle(NativeHandle* outHandle)
{
    outHandle->type = NativeHandleType::WebGPUTextureView;
...
Result DeviceImpl::createTextureView(ITexture* texture, const TextureViewDesc& desc, ITextureView** outView)
{
    ...
    WebGPUTextureViewDescriptor viewDesc = {};
    viewDesc.format =
    ...
    view->m_textureView = m_ctx.api.webgpuTextureCreateView(textureImpl->m_texture, &viewDesc);
