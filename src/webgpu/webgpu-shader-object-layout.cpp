...
inline WebGPUTextureViewDimension getViewDimension(SlangResourceShape shape)
{
    switch (shape)
    {
...
inline WebGPUTextureSampleType getSampleType(slang::TypeReflection* type)
{
...
)
{
    ...
    for (SlangInt bindingRangeIndex = 0; bindingRangeIndex < bindRangeCount; ++bindingRangeIndex)
    {
        ...
        for (SlangInt j = 0; j < descriptorRangeCount; ++j)
        {
            ...
            entry.visibility = WebGPUShaderStage_Vertex | WebGPUShaderStage_Fragment | WebGPUShaderStage_Compute;
            ...
            case slang::BindingType::Sampler:
                ...
                entry.sampler.type = WebGPUSamplerBindingType_Filtering;
                break;
            ...
            case slang::BindingType::MutableTexture:
                // WebGPUStorageTextureAccess_Undefined = 0x00000000,
                // WebGPUStorageTextureAccess_WriteOnly = 0x00000001,
                // WebGPUStorageTextureAccess_ReadOnly = 0x00000002,
                // WebGPUStorageTextureAccess_ReadWrite = 0x00000003,
                entry.storageTexture.access = WebGPUStorageTextureAccess_Undefined;
                entry.storageTexture.format = WebGPUTextureFormat_RGBA8Unorm;
            ...
            case slang::BindingType::RawBuffer:
                // entry.visibility = WebGPUShaderStage_Fragment | WebGPUShaderStage_Compute;
                entry.buffer.type = WebGPUBufferBindingType_ReadOnlyStorage;
                break;
            case slang::BindingType::MutableTypedBuffer:
            case slang::BindingType::MutableRawBuffer:
                entry.visibility = WebGPUShaderStage_Fragment | WebGPUShaderStage_Compute;
                entry.buffer.type = WebGPUBufferBindingType_Storage;
                break;
            ...
            case slang::BindingType::RayTracingAccelerationStructure:
                break;
            case slang::BindingType::ConstantBuffer:
                entry.buffer.type = WebGPUBufferBindingType_Uniform;
                break;
            default:
                break;
            }
...
ShaderObjectLayoutImpl::~ShaderObjectLayoutImpl()
{
    ...
    {
        getDevice()->m_ctx.api.webgpuBindGroupLayoutRelease(descSetInfo.bindGroupLayout);
    }
}
...
Result ShaderObjectLayoutImpl::_init(const Builder* builder)
{
    ...
    // Create WebGPUBindGroupLayout for all descriptor sets.
    for (auto& descriptorSetInfo : m_descriptorSetInfos)
    {
        WebGPUBindGroupLayoutDescriptor bindGroupLayoutDesc = {};
        ...
        descriptorSetInfo.bindGroupLayout =
            device->m_ctx.api.webgpuDeviceCreateBindGroupLayout(device->m_ctx.device, &bindGroupLayoutDesc);
...
RootShaderObjectLayoutImpl::~RootShaderObjectLayoutImpl()
{
    if (m_pipelineLayout)
    {
        m_device->m_ctx.api.webgpuPipelineLayoutRelease(m_pipelineLayout);
    }
}
...
Result RootShaderObjectLayoutImpl::_init(const Builder* builder)
{
    ...
    // Now call WebGPU API to create a pipeline layout.
    WebGPUPipelineLayoutDescriptor pipelineLayoutDesc = {};
    ...
    m_pipelineLayout = m_device->m_ctx.api.webgpuDeviceCreatePipelineLayout(m_device->m_ctx.device, &pipelineLayoutDesc);
