...
struct PrettyWriter
{
    ...
    void writeRaw(std::string_view str) { m_buffer += str; }
    void writeRawChar(int c) { m_buffer += char(c); }
    ...
    void dedent()
    {
        SLANG_RHI_ASSERT(m_indent > 0);
        m_indent--;
    }

    /// Write taking into account any CR that might be in a string
    ...
    void write(char const* text, size_t length) { write(std::string_view(text, length)); }

    /// Write the string as an escaped string
    void writeEscapedString(std::string_view str);
    void writeEscapedString(const char* str)
    {
        if (str)
            writeEscapedString(std::string_view(str));
    }

    /// Call before items in a comma-separated JSON list to emit the comma if/when needed
    void maybeComma();

    ThisType& operator<<(std::string_view str)
    {
        write(str);
        return *this;
    }
    ...
    std::string toString() { return m_buffer; }
...
void PrettyWriter::write(std::string_view str)
{
    ...
    while (start < end)
    {
        ...
        {
            adjust();
            writeRaw(std::string_view(start, std::distance(start, cur)));
        }
...
void PrettyWriter::writeEscapedString(std::string_view str)
{
    adjust();
    writeRawChar('"');
    // Write string with JSON escape codes
    for (char c : str)
    {
        switch (c)
        {
...
)
{
    ...
    if (categoryCount)
    {
        ...
        for (uint32_t cc = 0; cc < categoryCount; ++cc)
        {
            ...
            // Note: both `request` and `entryPointIndex` may be invalid here, but that should just make the function
            // return a failure.
            bool used = false;
            bool usedAvailable =
                spIsParameterLocationUsed(request, entryPointIndex, 0, category, space, index, used) == SLANG_OK;
    ...
    if (auto semanticName = var->getSemanticName())
    {
        writer.maybeComma();
        writer << "\"semanticName\": ";
        writer.writeEscapedString(semanticName);
...
static void emitReflectionNameInfoJSON(PrettyWriter& writer, char const* name)
{
    ...
    writer << "\"name\": ";
    writer.writeEscapedString(name);
}
...
static void emitReflectionTypeLayoutInfoJSON(PrettyWriter& writer, slang::TypeLayoutReflection* typeLayout)
{
    ...
    case slang::TypeReflection::Kind::Pointer:
    {
        ...
        SLANG_RHI_ASSERT(valueTypeLayout);
        ...
        if (typeName && typeName[0])
        {
            ...
            writer.writeEscapedString(typeName);
        }
        else
        {
            // TODO(JS): We will need to generate name that we will associate with this type
            // as it doesn't seem to have one
            writer.writeEscapedString("unknown name!");
            SLANG_RHI_ASSERT(!"Doesn't have an associated name");
        }
...
static void emitReflectionTypeLayoutJSON(PrettyWriter& writer, slang::TypeLayoutReflection* typeLayout)
{
    CommaTrackerRAII commaTracker(writer);
    writer << "{\n";
    writer.indent();
...
static void emitReflectionTypeParamJSON(PrettyWriter& writer, slang::TypeParameterReflection* typeParam)
{
    writer << "{\n";
    writer.indent();
...

    {
        ...
        if (count)
        {
            ...
            for (SlangUInt i = 0; i < count; ++i)
            {
                ...
                writer.writeEscapedString(std::string_view(chars, charsCount));
                writer << ": ";
...
{
    PrettyWriter writer;
    emitReflectionJSON(writer, nullptr, programLayout);
