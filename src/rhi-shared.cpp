...
Result ShaderObjectBase::_getSpecializedShaderObjectType(ExtendedShaderObjectType* outType)
{
    ...
    {
        m_shaderObjectType.slangType = getDevice()->slangContext.session->specializeType(
            _getElementTypeLayout()->getType(),
            specializationArgs.components.data(),
            specializationArgs.getCount()
        );
        m_shaderObjectType.componentID = getDevice()->shaderCache.getComponentId(m_shaderObjectType.slangType);
    }
    *outType = m_shaderObjectType;
    return SLANG_OK;
}
