...
)
{
    ...
    m_bindingData->rootParameters =
        m_allocator->allocate<BindingDataImpl::RootParameter>(specializedLayout->getTotalRootTableParameterCount());
    m_bindingData->rootParameterCount = specializedLayout->getTotalRootTableParameterCount();
    ...
    SLANG_RETURN_ON_FAIL(bindAsConstantBuffer(shaderObject, descriptorSet, rootOffset, specializedLayout));
    ...
    for (size_t i = 0; i < entryPointCount; ++i)
    {
        ...
        SLANG_RETURN_ON_FAIL(bindAsConstantBuffer(entryPoint, descriptorSet, entryPointOffset, entryPointLayout));
    }
...
)
{
    ...
    SLANG_RETURN_ON_FAIL(bindAsValue(shaderObject, descriptorSet, inOffset, specializedLayout));
    return SLANG_OK;
}
...
)
{
    ...
    //
    SLANG_RETURN_ON_FAIL(bindAsConstantBuffer(shaderObject, descriptorSet, subOffset, specializedLayout));
...
)
{
    ...
    for (const auto& bindingRangeInfo : specializedLayout->m_bindingRanges)
    {
        ...
        case slang::BindingType::MutableTypedBuffer:
        {
            ...
            ResourceState requiredState = isSrv ? ResourceState::ShaderResource : ResourceState::UnorderedAccess;
            uint32_t resourceIndex = rangeOffset.resource + baseIndex;
            uint32_t rootParamIndex = rangeOffset.rootParam + baseIndex;
            for (uint32_t i = 0; i < count; ++i)
            {
                const ResourceSlot& slot = shaderObject->m_slots[slotIndex + i];
                ...
                {
                    SLANG_RHI_ASSERT(rootParamIndex + i < m_bindingData->rootParameterCount);
                    m_bindingData->rootParameters[rootParamIndex + i] =
                        isSrv ? createRootSRV(rootParamIndex + i, buffer->getDeviceAddress() + slot.bufferRange.offset)
                              : createRootUAV(rootParamIndex + i, buffer->getDeviceAddress() + slot.bufferRange.offset);
                }
                else
                {
                    if (isTyped)
        ...
        case slang::BindingType::RayTracingAccelerationStructure:
        {
            ...
            for (uint32_t i = 0; i < count; ++i)
            {
                ...
                if (bindingRangeInfo.isRootParameter)
                {
                    SLANG_RHI_ASSERT(rootParamIndex + i < m_bindingData->rootParameterCount);
                    m_bindingData->rootParameters[rootParamIndex + i] =
                        createRootSRV(rootParamIndex + i, as->getDeviceAddress());
                }
                else
                {
                    SLANG_RHI_ASSERT(resourceIndex + i < descriptorSet.resources.count);
    ...
    for (const auto& subObjectRange : specializedLayout->m_subObjectRanges)
    {
        ...
        case slang::BindingType::ConstantBuffer:
        {
            ...
            for (uint32_t i = 0; i < count; ++i)
            {
                ...
                SLANG_RETURN_ON_FAIL(bindAsConstantBuffer(subObject, descriptorSet, objOffset, subObjectLayout));
        ...
        case slang::BindingType::ExistentialValue:
            if (subObjectLayout)
            {
                ...
                for (uint32_t j = 0; j < bindingRangeInfo.count; j++)
                {
                    ...
                    SLANG_RETURN_ON_FAIL(bindAsValue(subObject, descriptorSet, objOffset, subObjectLayout));
                    objOffset += rangeStride;
                }
            }
            break;
