...
struct Submitter
{
    ...
    Submitter(ID3D12Device* device, ID3D12GraphicsCommandList* commandList, TransientResourceHeapImpl* transientHeap)
        : m_device(device)
    ...
    virtual void uploadData(BufferImpl* buffer, size_t offset, size_t size, void const* data);
...
struct GraphicsSubmitter : public Submitter
{
    ...
    GraphicsSubmitter(
        ...
        : Submitter(device, commandList, transientHeap)
    {
    }
};
