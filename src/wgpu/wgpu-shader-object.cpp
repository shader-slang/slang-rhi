...
{
    SLANG_RHI_ASSERT(bindingSet < builder.entries.size());
    builder.entries[bindingSet].push_back(write);
}
...
)
{
    ...
    bindingCache->bindingData.push_back(bindingData);

    bindGroupLayouts = specializedLayout->m_bindGroupLayouts;
...
{
    bindingData->bindGroupCount = entries.size();
    bindingData->bindGroups = allocator->allocate<WGPUBindGroup>(bindingData->bindGroupCount);

    for (size_t i = 0; i < entries.size(); ++i)
    {
        WGPUBindGroupDescriptor desc = {};
        desc.layout = bindGroupLayouts[i];
        desc.entries = entries[i].data();
        desc.entryCount = (uint32_t)entries[i].size();
        WGPUBindGroup bindGroup = device->m_ctx.api.wgpuDeviceCreateBindGroup(device->m_ctx.device, &desc);
        if (!bindGroup)
        {
            return SLANG_FAIL;
        }
        bindingData->bindGroups[i] = bindGroup;
    }
    return SLANG_OK;
}
...
)
{
    ...
    offset.bindingSet = (uint32_t)entries.size();
    offset.binding = 0;
    ...
    SLANG_RHI_ASSERT(offset.bindingSet < (uint32_t)entries.size());
...
)
{
    ...
    SLANG_RETURN_ON_FAIL(constantBufferPool->allocate(bufferSize, allocation));
