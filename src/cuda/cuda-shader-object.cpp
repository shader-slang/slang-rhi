    ...
    for (const auto& bindingRange : specializedLayout->m_bindingRanges)
    {
        ...
        case slang::BindingType::Texture:
            for (uint32_t i = 0; i < count; ++i)
            {
                ...
                memcpy(dst + uniformOffset + (i * 16), &handle, sizeof(handle));
            }
            break;
        ...
        case slang::BindingType::MutableTypedBuffer:
            for (uint32_t i = 0; i < count; ++i)
            {
                ...
                memcpy(dst + uniformOffset + (i * 16), &dataPtr, sizeof(dataPtr));
                memcpy(dst + uniformOffset + (i * 16) + 8, &dataSize, sizeof(dataSize));
            }
            break;
        ...
        case slang::BindingType::RayTracingAccelerationStructure:
            for (uint32_t i = 0; i < count; ++i)
            {
                ...
                memcpy(dst + uniformOffset + (i * 8), &handle, sizeof(handle));
            }
            break;
#endif
        }
