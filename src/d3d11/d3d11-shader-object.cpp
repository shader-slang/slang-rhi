...
)
{
    ...
    bindingData = allocator->allocate<BindingDataImpl>();
    ...
    for (const auto& bindingRange : specializedLayout->m_bindingRanges)
    {
        ...
        case slang::BindingType::Texture:
            for (uint32_t i = 0; i < count; ++i)
            {
                ...
                bindingData->srvs[registerIndex] = textureView->getSRV();
                bindingData->srvCount = max(bindingData->srvCount, registerIndex + 1);
            }
            break;
        case slang::BindingType::MutableTexture:
            for (uint32_t i = 0; i < count; ++i)
            {
                ...
                bindingData->uavs[registerIndex] = textureView->getUAV();
                bindingData->uavCount = max(bindingData->uavCount, registerIndex + 1);
            }
            break;
        case slang::BindingType::Sampler:
            for (uint32_t i = 0; i < count; ++i)
            {
                ...
                bindingData->samplers[registerIndex] = sampler->m_sampler;
                bindingData->samplerCount = max(bindingData->samplerCount, registerIndex + 1);
            }
            break;
        ...
        case slang::BindingType::TypedBuffer:
            for (uint32_t i = 0; i < count; ++i)
            {
                ...
                uint32_t registerIndex = bindingRange.registerOffset + offset.srv + i;
                SLANG_RHI_ASSERT(registerIndex < D3D11_COMMONSHADER_INPUT_RESOURCE_SLOT_COUNT);
                bindingData->srvs[registerIndex] = buffer->getSRV(slot.format, slot.bufferRange);
                bindingData->srvCount = max(bindingData->srvCount, registerIndex + 1);
            }
            break;
        ...
        case slang::BindingType::MutableTypedBuffer:
            for (uint32_t i = 0; i < count; ++i)
            {
                ...
                uint32_t registerIndex = bindingRange.registerOffset + offset.uav + i;
                SLANG_RHI_ASSERT(registerIndex < D3D11_PS_CS_UAV_REGISTER_COUNT);
                bindingData->uavs[registerIndex] = buffer->getUAV(slot.format, slot.bufferRange);
                bindingData->uavCount = max(bindingData->uavCount, registerIndex + 1);
            }
            break;
        }
...
)
{
    ...
    SLANG_RETURN_ON_FAIL(constantBufferPool->allocate(size, allocation));
    ...
    bindingData->cbvsBuffer[ioOffset.cbv] = allocation.buffer->m_buffer;
    bindingData->cbvsFirst[ioOffset.cbv] = allocation.offset / 16;
    bindingData->cbvsCount[ioOffset.cbv] = ((size + 15) / 16) * 16;
    bindingData->cbvCount = max(bindingData->cbvCount, ioOffset.cbv + 1);
    ioOffset.cbv++;
