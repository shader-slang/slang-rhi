...
GPU_TEST_CASE("bindless-descriptor-handles", D3D12 | Vulkan)
{
    ...
    {
        SKIP("Bindless descriptor handles are not supported");
    }
    ...
    DescriptorHandle resultHandle;
    {
        ...
        REQUIRE_CALL(result->getDescriptorHandle(
            DescriptorHandleAccess::ReadWrite,
            Format::Undefined,
            kEntireBuffer,
            &resultHandle
        ));
        CHECK(resultHandle.type == DescriptorHandleType::RWBuffer);
    }

    {
        ...
        cursor["result"].setDescriptorHandle(resultHandle);
