...
GPU_TEST_CASE("buffer-barrier", ALL)
{
    Shader programA;
    Shader programB;
    REQUIRE_CALL(loadProgram(device, programA.program, "test-buffer-barrier", {"computeA"}, &programA.reflection));
    REQUIRE_CALL(loadProgram(device, programB.program, "test-buffer-barrier", {"computeB"}, &programB.reflection));
