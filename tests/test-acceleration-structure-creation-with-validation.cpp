...
GPU_TEST_CASE("acceleration-structure-creation-with-validation", Vulkan)
{
    // Ensure that GPU-AV does not assert when using an acceleration-structure
    DeviceExtraOptions deviceExtraOptions{};
    deviceExtraOptions.forceValidation = true;
    deviceExtraOptions.validationMessageSeverityToEmit = ValidationMessageSeverity::Error;
    device = createTestingDevice(ctx, ctx->deviceType, false, &deviceExtraOptions);
    auto m_device = device;
    if (!m_device->hasFeature(Feature::RayTracing) && device->hasFeature(Feature::RayTracingValidation))
        SKIP("lacking support for raytracing validation");
