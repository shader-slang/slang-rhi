...
GPU_TEST_CASE("bindless-buffers", D3D12 | Vulkan)
{
    ...
    REQUIRE_CALL(loadProgram(device, shaderProgram, "test-bindless-buffers", {"computeMain"}, &slangReflection));
...
GPU_TEST_CASE("bindless-textures", D3D12 | Vulkan | CUDA)
{
    ...
    ComPtr<IShaderProgram> shaderProgram;
    slang::ProgramLayout* slangReflection = nullptr;
    REQUIRE_CALL(loadProgram(device, shaderProgram, "test-bindless-textures", {"computeMain"}, &slangReflection));
