...
GPU_TEST_CASE("texture-allocation-info-bc7", D3D12 | Vulkan | Metal | CUDA)
{
    TextureDesc desc;
    desc.type = TextureType::Texture2D;
    desc.size = {256, 256, 1};
    desc.format = Format::BC7Unorm;
    desc.mipCount = 1;
    desc.arrayLength = 1;
    desc.usage = TextureUsage::ShaderResource;
    ...
    // BC7 is 16 bytes per 4x4 block = 1 byte per pixel
    // Size should be at least (width/4) * (height/4) * 16 bytes per block
    CHECK_GE(size, (256 / 4) * (256 / 4) * 16);
    // Alignment should be non-zero and a power of 2
    CHECK_GT(alignment, 0);
    CHECK_EQ(alignment & (alignment - 1), 0);
}
