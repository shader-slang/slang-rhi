...
static const AccelerationStructureAABB kAabbData[kAabbCount] = {
    {-0.5f, -0.5f, 0.5f, 0.5f, 0.5f, 1.0f},
};

struct RayTracingHitObjectIntrinsicsTestCustomGeometry
{
    IDevice* device;
    ...
    )
    {
        ...
        SLANG_RETURN_ON_FAIL(module->findEntryPointByName(intersectionName, entryPoint.writeRef()));
        componentTypes.push_back(entryPoint);
    ...
    )
    {
        ...
        // Build bottom level acceleration structure.
        {
            // TODO(Kincaid): Probably just pull this out into an overridable method
            AccelerationStructureBuildInput buildInput = {};
        ...
        hitGroups[0].hitGroupName = hitgroupNames[0];
        hitGroups[0].closestHitEntryPoint = closestHitName;
        hitGroups[0].intersectionEntryPoint = intersectionName;
...
GPU_TEST_CASE("ray-tracing-hitobject-query-hit-kind-custom", ALL & ~CUDA)
{
    ...
    RayTracingHitObjectIntrinsicsTestCustomGeometry test;
    test.init(device);
    test.run(
        "test-ray-tracing-hitobject-intrinsics",
        "rayGenShaderQueryHitKindCustom",
        "intersectionReportHitWithKind",
        "closestHitNOP",
        "missNOP"
    );
    test.checkQueryAndInvokeResults();
